            ...
            <div className="proper-modal-header" style={{ position: 'relative', width: '100%', height: '380px', overflow: 'hidden' }}>
              {(!artist.img && !artist.image) || imageError ? (
                <Image
                  ...
                  alt={artist.name} fill sizes="(max-width: 768px) 100vw, 800px" style={{ objectFit: 'cover', objectPosition: 'top center' }}  />
              ) : (
                <Image
                  src={typeof artist.img || artist.image === "object" ? artist.img || artist.image?.src : artist.img || artist.image}
                  alt={artist.name} fill sizes="(max-width: 768px) 100vw, 800px" style={{ objectFit: 'cover', objectPosition: 'top center' }} onError={() => setImageError(true)}
                />
              )}
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px' }}>
                        {artist.galleryImages.map((imgUrl, i) => (
                          <div key={i} style={{ position: 'relative', height: '100px', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
                            <Image src={typeof imgUrl === "object" ? imgUrl?.src : imgUrl} style={{ objectFit: "cover" }} fill sizes="(max-width: 768px) 150px, 150px" alt={`${artist.name} gallery ${i+1}`}  />
                          </div>
                        ))}
                      </div>
                    )}

      ...
      <Dialog open={deleteModalOpen} onOpenChange={setDeleteModalOpen}>
        <DialogContent className="max-w-md rounded-[32px] border-none shadow-2xl p-0 max-h-[90dvh] overflow-y-auto">
          ...
          <div className="p-8 bg-slate-50 flex flex-col gap-3">
            ...
            <button onClick={async () => { if(await confirmAction('Admin Verification Required', 'Are you absolutely sure you want to delete ALL client requests? This cannot be undone.', 'danger')) handleDeleteAll(); }} className="w-full h-11 rounded-xl bg-black text-white font-bold text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
